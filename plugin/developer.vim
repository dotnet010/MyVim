if exists('g:loaded_developer')
    finish
endif
let g:loaded_developer= 1
...
function s:AddTitle()
    ...
    normal o
    call setline('.',noTypeChar.preChar.'    Developer: '.g:developer_developer)
    normal o
    call setline('.',noTypeChar.preChar.'        Email: '.g:developer_email)
    normal o
    call setline('.',noTypeChar.preChar.'     HomePage: '.g:developer_homepage)
    normal o
    call setline('.',noTypeChar.preChar.'      Version: '.g:developer_version)
    normal o
    call setline('.',noTypeChar.preChar.'   LastChange: '.strftime("%Y-%m-%d %H:%M:%S"))
    normal o
    call setline('.',noTypeChar.preChar.'      Explain: ')
    let gotoLn = line('.')
    normal o
...
function s:TitleDet()
    ...
    while n < 20
		...
		if line =~ '^.*Version: \d\{1,'.g:developer_version_length.'}.\{0,1}\d\{0,'.g:developer_version_length.'}.\{0,1}\d\{0,'.g:developer_version_length.'}$'
			"let newline=substitute(line,'/\(\d*\)$/\',submatch(0)+1,'g')
			"s/\(\d\)$/\=submatch(0)+1/
			exe n.'s/\(\d*\)$/\=submatch(0)+1'
			"echo n.'s/\(\d*\)$/\=submatch(0)+1'
			"call setline(n,newline)
			let updated = 1
        endif
        let n = n + 1
...
command! -nargs=0 DeveloperInfoDetect :call s:TitleDet()
